...
```bash
zcat S1_10_L001_R1_001.fastq.gz | grep -c "^@"
```
Oppure possiamo usare:
...
## Rinominiamo tutti i files in un formato più semplice:
...
do
    sample=$(echo "$f" | cut -d'_' -f1)
    read=$(echo "$f" | cut -d'_' -f4)
   echo "mv "$f" "${sample}_${read}.fastq.gz""
done
```
```bash
for f in *.fastq.gz
...
```
## Apriamo il nuovo file con un editor di testo

```bash
nano subset_S1_R1.fastq
```
